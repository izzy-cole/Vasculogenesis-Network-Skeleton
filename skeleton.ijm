...
for (s=0;s<stages.length;s++){
	...
	for (n = 1; n <= n_s.length; n++) {
		for (c = 0; c< conditions.length; c++) {
			...
			ref_range = 208-34;
			scaled_contrast = ((light-dark)/ref_range)*120;
			print("Scaled contrast is: "+scaled_contrast);
			...
			run("Remove Outliers...", "radius=7 threshold=50 which=Dark");
			run("Remove Outliers...", "radius=7 threshold=50 which=Bright");
